...
### 1.1 반올림 규칙(고정)
- **Math.ceil 고정**: 모든 XP 계산은 올림(`ceil`)으로 통일한다.
  - 저장/업로드 데이터에는 별도 `rounding` 필드를 넣지 않는다.
  - UI 미리보기/테스트 역시 `ceil` 기준으로 표시된다.
...
> **데이터 주의**: 모든 계산은 `Math.ceil`로 고정되므로 저장 스키마에 `rounding` 필드를 포함하지 않는다.
